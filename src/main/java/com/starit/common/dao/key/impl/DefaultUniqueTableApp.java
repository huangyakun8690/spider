...
import com.starit.common.dao.key.CacheValue;
import com.starit.common.dao.key.UniqueException;
...
public class DefaultUniqueTableApp implements UniqueTableApp {
	...
	private CacheValue getCurrCode(String name) {
		...
		try {
			Long code = jdbcTemplate.queryForObject(this.selectSQL, new Object[] {name},Long.class);
			value = new CacheValue();
			value.setMinVal(code + 1);
		} catch (EmptyResultDataAccessException e) {
			logger.debug(name + " 娌℃湁鎵惧埌璁板綍");
		} catch (Exception e) {
			throw new UniqueException(name + " 鑾峰彇涓婚敭澶辫触");
		}
		return value;
	}
